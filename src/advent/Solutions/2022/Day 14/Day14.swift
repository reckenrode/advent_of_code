...
extension Solutions.Year2022 {
    struct Day14: ParsableCommand {
        ...
        @Flag(help: "display the grid with sand") var showGrid: Bool = false
        @Flag(help: "display grid after each unit of sand lands") var debug: Bool = false
        ...
        func run() throws {
            ...
            while let needMore = sandgrid.process() {
                if needMore {
                    ...
                    if self.debug {
                        print(sandgrid.description)
                        print("\(sandCount) units of sand have landed")
                    } else {
            ...
            if self.showGrid { print(sandgrid.description) }
        }
    }
}
